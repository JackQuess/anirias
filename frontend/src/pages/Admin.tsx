  ...
  return (
    <div className="space-y-12 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className="px-3 py-1 bg-brand-red/10 text-brand-red text-[9px] font-black uppercase tracking-widest rounded-lg border border-brand-red/20">v2.4.0 Stable</span>
        ...
        <div className="flex gap-4">
           <div className="text-right">
              ...
              <p className="text-xl font-mono text-white font-bold">{new Date().toLocaleTimeString('tr-TR')}</p>
           </div>
        </div>
      </header>
            ...
            {featuredAnimes?.map((anime) => (
               <div key={anime.id} className="group relative aspect-video rounded-2xl overflow-hidden border border-white/10 hover:border-brand-red transition-all">
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                     <p className="text-[10px] font-black text-white uppercase truncate">{getDisplayTitle(anime.title)}</p>
                     <p className="text-[8px] text-gray-400 font-bold uppercase mt-1">SLIDER AKTİF</p>
                  </div>
                  <button 
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
          {[
            ...
            <div key={stat.label} className={`p-8 rounded-[2.5rem] border backdrop-blur-sm relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300 ${stat.bg}`}>
               <div className="flex justify-between items-start mb-4">
                  <div className="w-12 h-12 rounded-2xl bg-black/20 flex items-center justify-center text-2xl">
                     {stat.icon}
                  </div>
                  <span className={`px-3 py-1 rounded-lg text-[9px] font-black bg-black/20 ${stat.color}`}>
                     {stat.change}
                  </span>
               </div>
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">{stat.label}</p>
               <p className="text-4xl font-black text-white tracking-tight">{stat.value.toLocaleString()}</p>
               
               {/* Decorative Background Icon */}
               <div className="absolute -right-6 -bottom-6 text-9xl opacity-[0.03] grayscale group-hover:opacity-[0.07] transition-opacity rotate-12 pointer-events-none">
                 {stat.icon}
               </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 bg-[#0a0a0a] border border-white/5 p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
           <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 pointer-events-none" />
           <div className="flex items-center justify-between mb-10 relative z-10">
              <div>
                 <h3 className="text-xl font-black text-white uppercase tracking-tighter italic">SUNUCU <span className="text-brand-red">YÜKÜ</span></h3>
                 <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">CANLI METRİKLER (CPU / RAM)</p>
              </div>
              <div className="flex gap-2">
                 <span className="w-3 h-3 bg-brand-red rounded-full animate-pulse" />
                 <span className="text-[10px] font-black text-brand-red uppercase tracking-widest">LIVE</span>
              </div>
           </div>
           ...
           <div className="flex justify-between mt-6 pt-6 border-t border-white/5 relative z-10">
              <div className="text-center w-full border-r border-white/5">
                 <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest">CPU</p>
                 <p className="text-2xl font-black text-white">42%</p>
              </div>
              <div className="text-center w-full border-r border-white/5">
                 <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest">RAM</p>
                 <p className="text-2xl font-black text-white">6.2GB</p>
              </div>
              <div className="text-center w-full">
                 <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest">LATENCY</p>
                 <p className="text-2xl font-black text-brand-red">24ms</p>
              </div>
           </div>
        </div>

        {/* Quick Actions */}
        <div className="bg-[#0a0a0a] border border-white/5 p-10 rounded-[3rem] shadow-2xl flex flex-col relative overflow-hidden">
           <h3 className="text-xl font-black text-white uppercase tracking-tighter italic mb-8 relative z-10">HIZLI <span className="text-brand-red">İŞLEMLER</span></h3>
           
           <div className="grid grid-cols-2 gap-4 flex-1 relative z-10">
              <Link to="/admin/animes/new" className="bg-white/5 hover:bg-brand-red text-gray-400 hover:text-white border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center gap-3 transition-all group">
                 <span className="text-3xl group-hover:scale-110 transition-transform">➕</span>
                 <span className="text-[9px] font-black uppercase tracking-widest text-center">Anime Ekle</span>
              </Link>
              <Link to="/admin/import" className="bg-white/5 hover:bg-purple-600 text-gray-400 hover:text-white border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center gap-3 transition-all group">
                 <span className="text-3xl group-hover:scale-110 transition-transform">🤖</span>
                 <span className="text-[9px] font-black uppercase tracking-widest text-center">AI Import</span>
              </Link>
              <Link to="/admin/users" className="bg-white/5 hover:bg-blue-600 text-gray-400 hover:text-white border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center gap-3 transition-all group">
                 <span className="text-3xl group-hover:scale-110 transition-transform">👥</span>
                 <span className="text-[9px] font-black uppercase tracking-widest text-center">Üyeler</span>
              </Link>
              <Link to="/admin/analytics" className="bg-white/5 hover:bg-emerald-600 text-gray-400 hover:text-white border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center gap-3 transition-all group">
                 <span className="text-3xl group-hover:scale-110 transition-transform">📊</span>
                 <span className="text-[9px] font-black uppercase tracking-widest text-center">Raporlar</span>
              </Link>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         {/* System Status Table */}
         <div className="bg-brand-surface border border-white/5 p-10 rounded-[3rem]">
            <h3 className="text-sm font-black text-white uppercase tracking-widest italic mb-8 border-l-4 border-brand-red pl-4">SİSTEM DURUMU</h3>
            <div className="space-y-4">
               {[
                 ...
                 <div key={i} className="flex items-center justify-between p-5 bg-white/[0.02] rounded-2xl border border-white/5 hover:bg-white/[0.05] transition-colors">
                    <div className="flex items-center gap-4">
                       <div className={`w-2 h-2 rounded-full ${sys.color} animate-pulse`} />
                       <span className="text-[10px] font-black text-white uppercase tracking-widest">{sys.name}</span>
                    </div>
                    <div className="flex gap-6">
                       <span className="text-[10px] font-bold text-gray-500 uppercase">{sys.status}</span>
                       <span className="text-[10px] font-black text-white">{sys.uptime}</span>
                    </div>
                 </div>
               ))}
            </div>
         ...
         {/* Recent Activity */}
         <div className="bg-brand-surface border border-white/5 p-10 rounded-[3rem]">
            <h3 className="text-sm font-black text-white uppercase tracking-widest italic mb-8 border-l-4 border-brand-red pl-4">SON İŞLEMLER</h3>
            <div className="space-y-6 relative">
               ...
               {!activitiesLoading && activities?.length ? activities.map((log) => (
                 <div key={log.id} className="flex items-start gap-6 relative">
                    <div className="w-5 h-5 rounded-full bg-brand-black border-2 border-brand-red z-10 flex-shrink-0" />
                    <div>
                       <p className="text-[10px] font-black text-white uppercase tracking-wide">{log.action}</p>
                       <p className="text-xs text-gray-500 mt-1">{log.target} - <span className="text-brand-red">{log.user}</span></p>
                       <p className="text-[9px] text-gray-700 font-bold mt-1">{new Date(log.created_at).toLocaleString('tr-TR')}</p>
                    </div>
                 </div>
               )) : (!activitiesLoading && (
                 <p className="text-center text-gray-600 text-xs font-black uppercase tracking-widest">Kayıt bulunamadı</p>
               ))}
            </div>
         </div>
