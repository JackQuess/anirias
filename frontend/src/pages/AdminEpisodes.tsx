...
const AdminEpisodes: React.FC = () => {
  ...
  const handleFixSeasons = async () => {
    ...
    const confirmed = window.confirm('Bu işlem sezonları otomatik olarak yeniden düzenler. Devam edilsin mi?');
    if (!confirmed) return;
    ...
    setIsFixingSeasons(true);
    try {
      const res = await fetch(`${apiBase}/api/admin/fix-seasons`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-ADMIN-TOKEN': token
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Fix seasons failed');
      }
        ...
        <>
          ...
          {hasSeasons && (
            <div className="mb-6 flex justify-end">
              <button
                onClick={handleFixSeasons}
                disabled={isFixingSeasons}
                className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-200 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-blue-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {isFixingSeasons ? (
                  <>
