 ...
 *   "is_adult": boolean (optional; yoksa AniList'ten türetilir),
 *   "rating": string (optional),
...
router.post('/create-anime', async (req: Request, res: Response) => {
  try {
    ...
    let resolvedIsAdult: boolean;
    let contentRating: string | null = typeof rating === 'string' && rating.trim() ? rating.trim() : null;

    if (typeof bodyIsAdult === 'boolean') {
      resolvedIsAdult = bodyIsAdult;
    } else if (anilist_id && typeof anilist_id === 'number' && Number.isFinite(anilist_id)) {
      try {
        const m = await getAniListMedia(anilist_id);
        if (m) {
          const d = deriveAdultRatingFromAniListMedia(m);
          resolvedIsAdult = d.is_adult;
          if (!contentRating && d.rating) contentRating = d.rating;
        } else {
          resolvedIsAdult = false;
        }
      } catch (e) {
        console.warn('[create-anime] AniList adult derivation skipped:', e);
        resolvedIsAdult = false;
      }
    } else {
      resolvedIsAdult = false;
    ...
    if (contentRating || resolvedIsAdult) {
      const meta = await updateAnimeAdultMetadata(supabaseAdmin, data.id, {
        is_adult: resolvedIsAdult,
        contentLabel: contentRating,
      });
      if (!meta.ok) {
        console.warn('[create-anime] Adult metadata:', meta.message);
      }
    }
